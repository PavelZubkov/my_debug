...
var $;
(function ($) {
    class $my_debug_ftp extends $.$mol_object2 {
        list(next) {
        ...
        update_item(id) {
            ...
            if (item.value > 5) {
                console.log('update item start', id);
                $.$mol_fiber_defer(() => {
                    const next = this.item(id, { value: 0 });
                    console.log('update_item ok', next);
                    return next;
                });
            }
            return item;
    ...
    __decorate([
        $.$mol_mem
    ], $my_debug_ftp.prototype, "list", null);
    __decorate([
        $.$mol_mem_key
    ], $my_debug_ftp.prototype, "item", null);
    $.$my_debug_ftp = $my_debug_ftp;
    class $my_debug_server extends $.$mol_object2 {
        ftp() {
            return new $my_debug_ftp;
        }
        start() {
            return $.$mol_atom2_autorun(() => this.ftp().update());
        }
    }
    __decorate([
        $.$mol_mem
    ], $my_debug_server.prototype, "ftp", null);
    __decorate([
        $.$mol_mem
    ], $my_debug_server.prototype, "start", null);
    $.$my_debug_server = $my_debug_server;
    function $my_debug_bootstrap() {
        const arg = $.$mol_state_arg.value('--start');
        if (arg === null) {
            console.log('Provide "--start" option for start server');
            return;
        }
        const server = new $my_debug_server;
        $.$mol_fiber_root(() => {
            server.start();
            new $.$mol_after_timeout(1000, () => {
                server.ftp().item(3, { value: 1111 });
            });
        })();
    }
    $.$my_debug_bootstrap = $my_debug_bootstrap;
    $my_debug_bootstrap();
})($ || ($ = {}));
//debug.js.map
;
